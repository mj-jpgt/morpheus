...
**Open (two agents running as of this writing):**
- Real answerable-query count is still near zero; the ceiling is that ~19/28 of anything answerable is on trained targets, ~1/24 on genuinely held-out ones. Two tests in flight: (1) whether a composed multivariate readout over existing axes recovers untrained-pathway signal a single-axis rule misses — separates "no signal" from "wrong readout shape"; (2) whether P3's certified causal names predict which untrained, biologically-adjacent queries an axis can answer.
