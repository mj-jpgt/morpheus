...
- **The usable-view count falls from "12 of 12" to 7 of 12** (4/6 `rna_biology`, 3/6 `full_biology`) once those floors are also measured on both arms (1.019×/1.020× → 1.230×/1.422×, `programme_free`-carried). All five losses are D2 pairs. **No D2 arm (Hallmark or PBS) has ever had its own retraining floor measured on either training arm** — closing this (five same-seed retrains per D2 arm, GPU, ten runs) is now this paper's largest single open item and is the entire distance between 12/12 and 7/12. The one surviving D2 pair (`rna_biology` seed 44) clears by **0.61%** — flag this margin the way §5.4 limit 2's 5.6% pass (which broke at n=10) was flagged, if it is ever pushed to a second arm. **STILL OPEN as of 2026-08-05 10:30 UTC — attempted and blocked, not measured.** The protocol is fully specified (`NOTEBOOK_ENTRIES/PREDECLARED_d2_arm_same_seed_floor_20260805T1015Z.md`, reusing `p2_envelope_floors.py` unchanged, just pointed at D2's exports) and revised to **n=10 per arm**, matching the standard this project now uses after `limit2_breaks_at_ten_repeats_20260805T0330Z.md` showed n=5 was insufficient for the shape-matched momentum row. It could not be run: this checkout could not authenticate to the training box (host identity had changed since this project's records of it, and once accepted, this checkout's on-file credential was rejected — see `NOTEBOOK_ENTRIES/d2_arm_same_seed_floor_blocked_no_box_access_20260805T1030Z.md`). GPU occupancy is therefore unknown, not "checked and clear." The next agent with a working credential should read that entry's §5 and just execute the predeclaration — the falsifier thresholds (does the fragile 0.61% pass survive a floor ≥ 1.2380× on `rna_biology`) are already fixed and must not be re-chosen after the numbers exist.
