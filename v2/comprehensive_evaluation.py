...
def _rows_for(path, root, text_labels, text_vectors, hallmark, master, snv, cnv):
    ...
    if wsi is not None and rna is not None and immune and (tr&iv).sum()>=3 and (te&iv).sum()>=2:
    ...
    for name,table in (("snv_burden",snv),("cnv_burden",cnv)):
        ...
        if patient is not None and len(np.unique(label[tr]))==2 and len(np.unique(label[te]))==2:
    ...
    if wsi is not None and rna is not None and valid[te].sum()>=2:
