...
Values are mean $\pm$ s.d. over seeds $\{42,43,44\}$ (per-seed $\Delta$: $+53.7, +50.6, +55.7$).

**Secondary readout (ii) — confirmed: rank rises, specificity does not move.** A ridge probe
($z_{\text{biology}}\!\to\!$ regenerated held-out Hallmark targets, full test coverage $n=2530$) gives a
within-cancer Pearson of **$0.1366$ (baseline, rank $49$)** vs. **$0.1367$ (F-R2, rank $103$)** — and
pooled $0.217$ vs. $0.219$. Doubling the biology head's effective rank leaves molecular-prompting
specificity **statistically unchanged**, exactly the pre-registered prediction. This is the load-bearing
result of the coupling claim (C2): effective rank is *decoupled* from the confounded benchmark score, so
the collapse is a representational pathology the metric is structurally blind to — which is precisely why
a rank fingerprint is diagnostic where the benchmark is not. One caveat remains: this is the
`programme_only` regime (baseline plateaus at $\approx 50$ vs. the $\approx 47$ of the full multi-loss
