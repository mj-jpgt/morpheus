...
We ran the pre-registered ablation on the real held-out-cancer cohort ($n_{\text{test}}=2530$, seed 42, `programme_only` profile, 25 epochs, token budget 65{,}536, NVIDIA A100). Both arms start from the same initialization (biology-head effective rank $146.0$). The **baseline arm** (decorrelation weight $0.0$, i.e. only the per-dimension variance floor) collapses monotonically to an effective rank of **$68.0$**. The **F-R2 arm** (decorrelation weight $0.04$, with the F-R1 shared-geometry coupling) holds the biology head at **$101.1$** — a **$+33.1$ ($+49\%$) higher effective rank** that resists the collapse (Fig. 4). This confirms the falsifiable primary prediction (i): the off-diagonal covariance-decorrelation term measurably recovers biology-head rank on real data, where the per-dimension variance floor alone does not.
...
Two honest caveats. This run is single-seed (42) and `programme_only`; the baseline arm plateaus at $\approx 68$ rather than the $\approx 47$ of the full multi-loss `v21` export (T1), because the isolated-biology regime applies weaker collapse pressure. And the secondary prediction (ii) — that specificity is *unchanged* — is not tested by this rank-only run; re-measuring control-adjusted within-cancer specificity on both arms is the remaining step (multi-seed, both readouts, variance-floor negative control are queued). T1 establishes that the biology head loses *rank*, not merely per-dimension variance. A per-dimension variance floor ($\mathrm{relu}(1-\mathrm{std})$, weight 0.01) constrains marginal variances but leaves the off-diagonal covariance — and therefore the rank — unconstrained; it provably cannot prevent rank collapse. The prescriptive fix (C3) couples two changes: **(F-R1)** feed a single L2-normalized biology state to both the Gaussian-NLL heads and the rank-spreaders so they share geometry, and **(F-R2)** add a VICReg/Barlow-style off-diagonal feature-decorrelation term on the normalized 256-D $z_{\text{biology}}$ — the mean-squared off-diagonal of the centered batch covariance, weight $\approx 0.04$, active in the `full` and `programme_only` profiles under a minimum-batch guard.
